# BEAM v11.1 standard internal deployment

Use `START_STANDARD.bat` or `python RUN_BEAM.py --standard` for an internal deployment. The prepared presenter demo is loopback-only; `START_BEAM.bat` now starts that isolated demo. Do not expose patient data through a public demo deployment. For an internal pilot, run one backend worker with a persistent `BEAM_DATA_DIR`, place it behind the hospital's approved HTTPS reverse proxy and enable WebSocket forwarding for `/ws`.
